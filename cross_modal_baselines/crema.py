...
class DiscrnV2Dataset(BaseDataset):        
    def __init__(self, **kwargs):
        ...
        self.video_processor =BlipVideoEvalProcessor(n_frms=4, image_size=224)
        ...
        self.voxel_root = '/export/einstein-vision/crema_model_data/objaverse_feat_blip/points'
        self.pc_feat_root = '/export/einstein-vision/crema_model_data/objaverse_feat_blip/features'
    ...
    def __getitem__(self, index):
        ...
        for example,modality in zip(data["examples"], data['modalities']):
            ...
            if modality  == "video":    
                ...
                if pc_feat.shape[0] > 5000:
                    ...
                    pc_feat = torch.cat([pc_feat.mean(1), torch.zeros(5000 - pc_feat.shape[0], pc_feat.shape[-1])], dim=0)
        ...
        data["text_input"] = data["questions"][0]
        ...
        data["direct_answers"] = data["answers"]
...
def main():
    ...
    model = DiscrnCREMA.from_pretrained('pretrain_flant5xl')
    # TODO: add the correct path.
    mmqa_ckpt = '/export/home/CREMA/crema_initial.pth'
    model.load_mmqa(mmqa_ckpt)
    model.downstream_task  = 'oeqa'

    datasets = {
        "test": {"test": DiscrnV2Dataset()},
    }
