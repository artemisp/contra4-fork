...
class DiscrnV2Dataset(BaseDataset):        
    def __init__(self, **kwargs):
        super().__init__(None, None,None,[])
        self.modalities = ['video', 'audio', 'images', 'pc']
        self.audio_processor = BeatsAudioProcessor(model_name='iter3',  sampling_rate=16000, n_frames=2, frame_length=512, is_eval=False, pad_with_fbank=False, random_sample=False)
        ...
        self.annotation = json.load(open('../data/discrn_balanced.json'))
    ...
    def __getitem__(self, index):
        data = copy.deepcopy(self.annotation[index])
        image = []
        for example,modality in zip(data["examples"], data['modalities']):
            file = os.path.join(self.key2path[example["source"]]["directory"], self.key2path[example["source"]]["key2path"][str(example['id'])])
            if 'image' in modality: 
                file = Image.open(file).convert("RGB")
            mod_feats = getattr(self, f"{modality if 'image' not in modality else 'vis'}_processor")(file)
            if modality  == "video":    
                data['video'] = mod_feats
            elif modality == "image":
                data['image'] = mod_feats
            elif modality == "point_cloud" or modality == 'pc':
                data['pc'] = mod_feats
            elif modality == "audio":
                data["audio"] = mod_feats.to(torch.float32)
        data["text_input"] = "Question: "+data["questions"][0]
        data["text_input"]+= "Choose from:"+ ", ".join([f'Scene {index2letter(i)}' for i,c in enumerate(example)])
    ...
    def generate(
            ...
            for modality in curr_modalities:
                if modality in Blip2VicunaXInstruct.SEQUENCIAL_MODALITIES and getattr(self, f'{modality}_enc_name') in Blip2VicunaXInstruct.SEQUENCIAL_ENCODERS:
                    ...
                    if self.projection_only or getattr(self, f"projection_only_{modality}"):
                        if self.proj_dim != 1:
                                query_outputs[modality] = getattr(self, f"{modality}_projection")(reordered_embeds.mean(1,keepdim=True)).view(bs*num[modality], self.num_query_token[modality], -1)
                        else:
                    ...
                    if self.projection_only or getattr(self, f"projection_only_{modality}"):
                        if self.proj_dim != 1:
                            query_outputs[modality] = getattr(self, f"{modality}_projection")(embeds[modality].mean(1, keepdim=True)).reshape(bs, self.num_query_token[modality],-1)
                        else:
                            with self.maybe_autocast():
                                query_outputs[modality] = getattr(self, f"{modality}_projection")(embeds[modality]).reshape(bs, self.num_query_token[modality],-1)
                        continue  
        ...
        for i,modality in enumerate(curr_modalities):
            if modality in Blip2VicunaXInstruct.SEQUENCIAL_MODALITIES and getattr(self, f'{modality}_enc_name') in Blip2VicunaXInstruct.SEQUENCIAL_ENCODERS:
                if self.projection_only or getattr(self, f"projection_only_{modality}"):
                    if self.proj_dim != 1:
                        inputs_llm[modality] = getattr(self, f"{modality}_llm_proj")(query_outputs[modality].unsqueeze(1)).reshape(bs*num[modality], self.num_query_token[modality], -1)
                    else:
                        inputs_llm[modality] = getattr(self, f"{modality}_llm_proj")(query_outputs[modality].reshape(bs*num, self.num_query_token[modality], -1))
                    inputs_llm[modality] = inputs_llm[modality].reshape(bs, num[modality], self.num_query_token[modality], -1).view(bs, num[modality]*self.num_query_token[modality], -1)
                ...
                if self.projection_only or getattr(self, f"projection_only_{modality}"):
                    if self.proj_dim == 1:
                        inputs_llm[modality] = getattr(self, f"{modality}_llm_proj")(query_outputs[modality].mean(-1)).reshape(bs, self.num_query_token[modality], -1)
                    else:
                        inputs_llm[modality] = getattr(self, f"{modality}_llm_proj")(query_outputs[modality].reshape(bs, self.num_query_token[modality], -1))
                    atts_llm[modality] =  torch.ones(inputs_llm[modality].size()[:-1], dtype=torch.long).to(self.device)
                    continue
                inputs_llm[modality] = getattr(self, f"{modality}_llm_proj")(query_outputs[modality]['last_hidden_state'][:,:query_tokens[modality].size(1),:])
                atts_llm[modality] = torch.ones(inputs_llm[modality].size()[:-1], dtype=torch.long).to(self.device)
                    ...
                    for modality in ['video', 'audio']:
                        if self.clean_tokenization:
                            if self.prefix or pos > 1 or self.enumerate_inputs or modality == 'audio':
                                att_list.extend([torch.tensor(self.tokenized_cue[modality].attention_mask[:,1:]).to(self.device).repeat(atts_llm[modality].shape[0], 1), atts_llm[modality].view(bs,  num[modality], self.num_query_token[modality])[:, pos, :]])
                                inp_list.extend([self.emb_cue[modality][:,1:].to(self.device).repeat(inputs_llm[modality].shape[0], 1, 1), inputs_llm[modality].view(bs,  num[modality], self.num_query_token[modality], -1)[:, pos, :, :]])
                                continue
                        att_list.extend([torch.tensor(self.tokenized_cue[modality].attention_mask).to(self.device).repeat(atts_llm[modality].shape[0], 1), atts_llm[modality].view(bs,  num[modality], self.num_query_token[modality])[:, pos, :]])
                        inp_list.extend([self.emb_cue[modality].to(self.device).repeat(inputs_llm[modality].shape[0], 1, 1), inputs_llm[modality].view(bs,  num[modality], self.num_query_token[modality], -1)[:, pos, :, :]])
                else:
                    att_list.extend([atts_llm[modality].view(bs, num[modality], self.num_query_token[modality])[:, pos, :]])
                    inp_list.extend([inputs_llm[modality].view(bs, num[modality], self.num_query_token[modality], -1)[:, pos, :, :]])
        else:
            for modality in curr_modalities:
                if self.enumerate_inputs:
...
def main():
    ...
    cfg = { "run":{'task': 'gqa', 'lr_sched': 'linear_warmup_cosine_lr', 'init_lr': 0.0001, 'min_lr': 1e-05, 'warmup_lr': 1e-08, 'warmup_steps': 1000, 'weight_decay': 0.05, 'max_epoch': 100, 'batch_size_train': 1, 'batch_size_eval': 1, 'num_workers': 4, 'accum_grad_iters': 1, 'max_len': 5, 'min_len': 1, 'num_beams': 5, 'inference_method': 'generate', 'seed': 42, 'output_dir': 'results/xinstructblip', 'amp': True, 'evaluate': True, 'train_splits': ['train'], 'valid_splits': ['val'], 'test_splits': ['test'], 'device': 'cuda', 'world_size': 16, 'dist_url': 'env://', 'distributed': True, 'find_unused_parameters': True}}
    options = {"options": None, "cfg_path": "LAVIS/lavis/projects/xinstruct_blip/eval/discrn/image_3d_caption.yaml"}
