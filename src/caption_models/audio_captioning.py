...
parser.add_argument('--model_name', type=str, default="Qwen/Qwen2-Audio-7B-Instruct", help="huggingface model name to use")
...
audio_dir = os.getenv('AUDIOCAPS_DIR') # os.getenv('CLOTHO_DIR')

with ctx:
    formatted_inputs = {'text': [], 'index': [], 'image_path': []}
    for d in os.listdir(audio_dir):
        formatted_inputs['text'].append("Describe the audio.")
        formatted_inputs['index'].append(d.split('.')[0])
        formatted_inputs['image_path'].append(os.path.join(audio_dir, d))
    ...
    class QwenVL2AudioPredictor:
        ...
        def __call__(self, batch: Dict[str, np.ndarray]) -> Dict[str, list]:
            ...
            for message in batch:
                audio_frames = librosa.load(message['multimodal_data']['audio_path'], sr=None)
                ...
                prompt = ("<|im_start|>system\nYou are a helpful assistant.<|im_end|>\n"
                        "<|im_start|>user\n"
                        f"{audio_in_prompt}{message['prompt']}<|im_end|>\n"
                        "<|im_start|>assistant\n")
                inputs = {"prompt": prompt, "multi_modal_data": {"audio": audio_frames}}
                final_messages.append(inputs)                
                indices.append(message['index'])
                
            outputs = self.llm.generate(final_messages,
                                        SAMPLING_PARAMS,
                                        use_tqdm=False,
                                        )
    ...
    MODEL_MAP = {
        ...
        "Qwen/Qwen2-Audio-7B-Instruct": QwenVL2AudioPredictor
    }
    ...
    ds = ds.map(lambda x: {'prompt': PROMPT_TEMPLATE_MAP[model].format(x['text']), 
                        'multimodal_data': {"audio_path": x['image_path']}, 
                        'index': x['index']}
                ).materialize()
    ...
    # ds.write_json(os.path.join('./caption_models/results', f'clotho_captions_{args.strategy}.pkl'))
    ds.write_json(os.path.join('results', f'audiocaps_captions_{args.strategy}.pkl'))
