...
parser.add_argument('--model_name', type=str, default='OpenGVLab/InternVL2-8B', help="huggingface model name to use")
...
with ctx:
    data = json.load(open(f'../../data/final_data/test.json'))
    formatted_inputs = {'text': [], 'index': [], 'image_path': []}
    for d in data:
        for e in d['examples']:
            if e['source'] == 'coco_val':
                if e['id'] in formatted_inputs['index']:
                    continue
                formatted_inputs['text'].append("Describe the image.")
                formatted_inputs['index'].append(e["id"])
                e['image_path']  = os.path.join(os.getenv('COCO_DIR'), f'{str(e["id"]).zfill(12)}.jpg')
                formatted_inputs['image_path'].append(e['image_path'])
    ...
    ds = ds.map(lambda x: {'prompt': PROMPT_TEMPLATE_MAP[model].format(x['text']), 
                        'multi_modal_data': {"image": Image.open(x['image_path']).convert("RGB")}, 
                        'index': x['index']}
                ).materialize()
    ...
    ds.write_json(os.path.join('results', f'image_captions.pkl'))
