    ...
    def __getitem__(self, index):
        ...
        for example,modality in zip(data["examples"], data['modalities']):
            if modality == "audio":
                ...
                file = np.load(file_obj)['arr_0']
            elif modality == "image":
                    file = Image.open(os.path.join(self.coco_dir, f'{str(example["id"]).zfill(12)}.jpg'))
        ...
        data["text_input"] = "Question: "+question
...
class Contra4Xinstruct(Blip2VicunaXInstruct):
    def predict_answers(
        self,
        samples,
        num_beams=5,
...
def main():
    ...
    job_id = now()
    os.makedirs("results/xinstructblip_random", exist_ok=True)
    cfg = { "run":{'task': 'gqa', 'lr_sched': 'linear_warmup_cosine_lr', 'init_lr': 0.0001, 'min_lr': 1e-05, 'warmup_lr': 1e-08, 'warmup_steps': 1000, 'weight_decay': 0.05, 'max_epoch': 100, 'batch_size_train': 1, 'batch_size_eval': 1, 'num_workers': 4, 'accum_grad_iters': 1, 'max_len': 5, 'min_len': 1, 'num_beams': 5, 'inference_method': 'generate', 'seed': 42, 'output_dir': 'results/xinstructblip_random', 'amp': True, 'evaluate': True, 'train_splits': ['train'], 'valid_splits': ['val'], 'test_splits': ['test'], 'device': 'cuda', 'world_size': 16, 'dist_url': 'env://', 'distributed': True, 'find_unused_parameters': True}}
    ...
    datasets = {
        "val": {"val": Contra4Dataset()},
    }
    model = Contra4Xinstruct.from_pretrained(model_type='vicuna7b')
